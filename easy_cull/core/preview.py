"""Image rendering and preview cache management for EasyCull."""
...
def _default_cache_dir() -> Path:
    ...
    if (home := Path.home()) and (home / 'Library').exists():
        return home / 'Library' / 'Caches' / 'easy-cull'

    return Path.home() / '.cache' / 'easy-cull'
...
def _fallback_cache_dir() -> Path:
    fallback = Path(tempfile.gettempdir()) / 'easy-cull'
